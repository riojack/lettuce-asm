...
class RegisterMemoryFacade {
  ...
  public write(register: string, bytes: number): void {
    ...
    const addressToWriteTo: number | undefined = this.registerMemoryMap.get(register);
  ...
  public read(register: string): number {
    ...
    const addressToReadFrom: number | undefined = this.registerMemoryMap.get(register);
